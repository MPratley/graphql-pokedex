...
//eventually this will just be lots of SQL on top of mariadb or sqlite
//for now just a big json dumb in memory

module.exports.resolvers = {
  Query: {
    pokemon(obj, args, context, info) {
      return collection.filter(pokemons, args);
    },
  },
  Pokemon: {
    next_evolution(pokemon) {
      return pokemons.filter( 
        p => collection.find(pokemon.next_evolution, {num: p.num})
      )
    },
    prev_evolution(pokemon) {
      return pokemons.filter( 
        p => collection.find(pokemon.prev_evolution, {num: p.num})
      )
    }
  }
};
